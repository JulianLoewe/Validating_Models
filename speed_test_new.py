...
def main():
    ...
    if args.experiment == "VizDataset":
        ...
        for constraints_separate in [None]:
                ...
                for use_outer_join in [True,False]:
                    for optimization in [True, False]:
                        ...
                        for k in range(NUM_REPS):
                            try:
                                result = validation_engine_experiment(f"{shape_schema.name}_{api_config.replace('/','_')}_{constraints_separate}_{use_outer_join}_{optimization}_{k}", endpoint, api_config, shape_schema, n_constraints=nconstraints_dict[shape_schema.name], constraints_separate=constraints_separate, use_outer_join=use_outer_join, optimize_intermediate_results=optimization)
                                result.result()
